...
create or replace table {stg_dataset_name}.rotten_tomatoes1 as
  select *
...
# Task to execute the BigQuery SQL query
rotten_tomatoes1 = BigQueryInsertJobOperator(
    task_id="rotten_tomatoes1",
    configuration={
        "query": {
            "query": rotten_tomatoes1_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_data_source_rotten_tomatoes_sql = f"""
update {stg_dataset_name}.disney_movies
  set data_source = 'rotten_tomatoes_ai' where rotten_tomatoes is null
"""

# Task to execute the BigQuery SQL query
update_data_source_rotten_tomatoes = BigQueryInsertJobOperator(
    task_id="update_data_source_rotten_tomatoes",
    configuration={
        "query": {
            "query": update_data_source_rotten_tomatoes_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_rotten_tomatoes_sql = f"""
UPDATE {stg_dataset_name}.disney_movies 
SET rotten_tomatoes = CAST(
    REPLACE(
        (SELECT JSON_VALUE(ml_generate_text_llm_result, '$.rotten_tomatoes') 
         FROM {stg_dataset_name}.rotten_tomatoes1 
...
# Task to execute the BigQuery SQL query
update_rotten_tomatoes = BigQueryInsertJobOperator(
    task_id="update_rotten_tomatoes",
    configuration={
        "query": {
            "query": update_rotten_tomatoes_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
start >> create_remote_models_central >> wait1 >> gemini >> wait2 >> rotten_tomatoes1 >> wait3 >> update_data_source_rotten_tomatoes >> wait4 >> update_rotten_tomatoes >> end
