...
create or replace table {stg_dataset_name}.release_date1 as
  select *
...
# Task to execute the BigQuery SQL query
release_date1 = BigQueryInsertJobOperator(
    task_id="release_date1",
    configuration={
        "query": {
            "query": release_date1_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_data_source_release_date_sql = f"""
update {stg_dataset_name}.disney_movies
  set data_source = 'release_date_ai' where release_date is null
"""

# Task to execute the BigQuery SQL query
update_data_source_release_date = BigQueryInsertJobOperator(
    task_id="update_data_source_release_date",
    configuration={
        "query": {
            "query": update_data_source_release_date_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_release_date_sql = f"""
update {stg_dataset_name}.disney_movies 
set release_date = 
    (select cast(json_value(ml_generate_text_llm_result, '$.release_date') as DATE) 
     from {stg_dataset_name}.release_date1 
...
# Task to execute the BigQuery SQL query
update_release_date = BigQueryInsertJobOperator(
    task_id="update_release_date",
    configuration={
        "query": {
            "query": update_release_date_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
start >> create_remote_models_central >> wait1 >> gemini >> wait2 >> release_date1 >> wait3 >> update_data_source_release_date >> wait4 >> update_release_date >> end
