...
create or replace table {stg_dataset_name}.budget1 as
  select *
...
# Task to execute the BigQuery SQL query
budget1 = BigQueryInsertJobOperator(
    task_id="budget1",
    configuration={
        "query": {
            "query": budget1_sql,
...
update_data_source_budget_sql = f"""
update {stg_dataset_name}.disney_movies
  set data_source = 'budget_ai' where budget is null
"""

# Task to execute the BigQuery SQL query
update_data_source_budget = BigQueryInsertJobOperator(
    task_id="update_data_source_budget",
    configuration={
        "query": {
            "query": update_data_source_budget_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_budget_sql = f"""
update {stg_dataset_name}.disney_movies set budget =
  CAST((select json_value(ml_generate_text_llm_result, '$.budget')
   from {stg_dataset_name}.budget1
...
# Task to execute the BigQuery SQL query
update_budget = BigQueryInsertJobOperator(
    task_id="update_budget",
    configuration={
        "query": {
            "query": update_budget_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
start >> create_remote_models_central >> wait1 >> gemini >> wait2 >> budget1 >> wait3 >> update_data_source_budget >> wait4 >> update_budget >> end
