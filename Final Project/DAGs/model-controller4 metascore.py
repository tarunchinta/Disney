...
create or replace table {stg_dataset_name}.metascore1 as
  select *
...
# Task to execute the BigQuery SQL query
metascore1 = BigQueryInsertJobOperator(
    task_id="metascore1",
    configuration={
        "query": {
            "query": metascore1_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
update_data_source_metascore_sql = f"""
update {stg_dataset_name}.disney_movies
  set data_source = 'metascore_ai' where metascore is null
"""

# Task to execute the BigQuery SQL query
update_data_source_metascore = BigQueryInsertJobOperator(
    task_id="update_data_source_metascore",
    configuration={
        "query": {
            "query": update_data_source_metascore_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
# Task to execute the BigQuery SQL query
update_metascore = BigQueryInsertJobOperator(
    task_id="update_metascore",
    configuration={
        "query": {
            "query": update_metascore_sql,
            "useLegacySql": False,
        }
    },
    location=region,
...
start >> create_remote_models_central >> wait1 >> gemini >> wait2 >> metascore1 >> wait3 >> update_data_source_metascore >> wait4 >> update_metascore >> end
